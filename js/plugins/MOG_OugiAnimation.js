...
//=============================================================================
/*:
 * @plugindesc (v1.3) Ativa uma animação antes de executar a ação. Suporte a GIF.
 * @author Moghunter (modificado)
 *
 * @param For Enemies
 ...
 * @desc Extensão dos arquivos de imagem (png, gif, jpg, etc.)
 * @default png
 *
 * @help  
 * =============================================================================
 * +++ MOG - Ougi Animation (v1.3 - GIF Support) +++
 * By Moghunter 
 ...
 *
 * =============================================================================
 * Serão necessários 2 arquivos de imagem (podem ser GIF animados), essas imagens 
 * deverão estar na pasta /img/pictures/
 *
 * Actor_ID.png (ou .gif)
 * Actor_ID + SUFIX.png (ou .gif)
 *
 * Enemy_ID.png (ou .gif)
 * Enemy_ID + SUFIX.png (ou .gif) 
 *
 * Exemplo
 *
 * Actor_2.png
 * Actor_2_Ougi.png 
 *
 * Para usar GIF, altere o parâmetro "File Extension" para "gif" e coloque os
 ...
 * (v1.3g) - Suporte a GIF através de parâmetro de extensão.
 *          - Correção de centralização para qualquer tipo de imagem.
 *          - Correção de duplicação de extensão.
 */
...
　　var Moghunter = Moghunter || {}; 
	...
	Moghunter.ougi_file_extension = String(Moghunter.parameters['File Extension'] || "png").toLowerCase();
...
//=============================================================================
// ** Método auxiliar para carregar imagens com extensão personalizada
//=============================================================================
Scene_Battle.prototype.loadOugiImage = function(filename) {
    var ext = Moghunter.ougi_file_extension;
    // Для png используем стандартный метод, он сам добавит .png
    if (ext === "png") {
        return ImageManager.loadPicture(filename);
    } else {
        // Для остальных расширений используем loadBitmap, который не добавляет расширение автоматически
        return ImageManager.loadBitmap('img/pictures/', filename + '.' + ext);
    }
};
...
Scene_Battle.prototype.createOugi = function() {
    this._spriteOugi = [];
	var subject = $gameTemp._ougiData[1];
	var sufix = $gameTemp._ougiData[4];
	
	if (subject.isActor()) {
		var baseName = "Actor_" + subject._actorId;
		this._spriteOugi[0] = new Sprite(this.loadOugiImage(baseName + sufix));
		this._spriteOugi[1] = new Sprite(this.loadOugiImage(baseName));
	} else {
		var baseName = "Enemy_" + subject._enemyId;
		this._spriteOugi[0] = new Sprite(this.loadOugiImage(baseName + sufix));
		this._spriteOugi[1] = new Sprite(this.loadOugiImage(baseName));
	}
	...
	this._spriteOugi[2] = false; // флаг центрирования
};
...
Scene_Battle.prototype.ougiTerminate = function() {
	...
	$gameTemp._ougiData = [false,null,0,0,""];
};
...
Scene_Battle.prototype.updateOugi = function() {
  ...
  if (!this._spriteOugi[2] && this.ougiIsReady()) {this.centerOugi();}
