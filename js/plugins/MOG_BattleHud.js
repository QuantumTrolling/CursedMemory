...
/*:
 * @plugindesc (v5.05 *) Permite customizar o layout de batalha.
 * @author Moghunter
 *
 * @param -> MAIN <<<<<<<<<<<<<<<<<<<<<<<
 * @desc
 ...
 * - WHAT'S  NEW (v5.05)
 * ============================================================================
 * - Correção do bug de ativar zoom na face quando a opção é desativada. 
 *
 */
...
Battle_Hud.prototype.refresh_meter_flow = function(sprite, value, value_max, type, flow) {
    var ch = sprite.bitmap.height / 2;
    var fullWidth = sprite.bitmap.width;

    // === НУЛЕВОЕ ЗНАЧЕНИЕ ===
    if (value <= 0) {
        sprite.setFrame(0, type * ch, 0, ch);
        return;
    }

    // ширина шкалы
    var meter_rate = fullWidth * value / value_max;

    // === ПОЛНОЕ ЗНАЧЕНИЕ ===
    if (value >= value_max) {
        sprite.setFrame(0, type * ch, fullWidth, ch);
        return;
    }

    // === FLOW ===
    var maxFlow = fullWidth - meter_rate;
    if (flow > maxFlow) {
        flow = maxFlow;
    }
...
Battle_Hud.prototype.create_mp_meter = function() {
	...
	if (String(Moghunter.bhud_mp_meter_flow) === "true") {this._mp_flow[0] = true;
	    this._mp_flow[2] = this._mp_meter_img.width;
